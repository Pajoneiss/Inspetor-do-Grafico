...
from config import (
    ...
    MAX_POSITION_ADDS_PER_SYMBOL_PER_HOUR,
    MIN_STOP_LOSS_PCT,
    DEFAULT_LEVERAGE,
...
# ==================================================================
# 📊 POSITION TRACKING (for logging only, NOT for limiting AI)
# ==================================================================
_position_open_time: Dict[str, float] = {}  # symbol -> timestamp when position was opened


def track_position_open(symbol: str):
    """Track when a position was opened (logging only)"""
    _position_open_time[symbol] = time.time()
    print(f"[TRACK] 📍 Position opened: {symbol} at {datetime.now().strftime('%H:%M:%S')}")


def track_position_close(symbol: str):
    """Track when a position was closed (logging only)"""
    if symbol in _position_open_time:
        hold_time = (time.time() - _position_open_time[symbol]) / 60
        print(f"[TRACK] 📍 Position closed: {symbol} (held for {hold_time:.1f} min)")
        del _position_open_time[symbol]
    else:
        print(f"[TRACK] 📍 Position closed: {symbol}")
...
def _execute_place_order(action: Dict[str, Any], is_paper: bool, hl_client) -> None:
    ...
    if is_paper:
        ...
        track_position_open(symbol)  # Track for logging
        return
...
def _execute_close_position(action: Dict[str, Any], is_paper: bool, hl_client) -> None:
    ...
    if is_paper:
        ...
        track_position_close(symbol)  # Track for logging
        return True
    
    # LIVE execution - AI has full autonomy
