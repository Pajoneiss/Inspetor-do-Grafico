...
export default function ContextCard({ market, scanner, aiThinking, loading }: ContextCardProps) {
            ...
            <div className="space-y-3 py-2">
                <div>
                    <div className="text-[9px] font-bold text-profit uppercase mb-1 flex items-center gap-1">
                        <span className="w-1.5 h-1.5 bg-profit animate-pulse" /> Top Bullish
                    </div>
                    <div className="flex flex-wrap gap-1">
                        {top5.length ? top5.map(s => (
                            <span key={s.symbol} className="px-1.5 py-0.5 bg-profit/10 border border-profit/20 text-[10px] font-bold text-profit">
                ...
                <div>
                    <div className="text-[9px] font-bold text-loss uppercase mb-1 flex items-center gap-1">
                        <span className="w-1.5 h-1.5 bg-loss" /> Top Bearish
                    </div>
                    <div className="flex flex-wrap gap-1">
                        {bottom3.length ? bottom3.map(s => (
                            <span key={s.symbol} className="px-1.5 py-0.5 bg-loss/10 border border-loss/20 text-[10px] font-bold text-loss">
                                {s.symbol} <span className="opacity-60 text-[8px] ml-0.5">{s.score}</span>
                            </span>
                        )) : <span className="text-[10px] text-text-muted">No signals</span>}
                    </div>
    ...
    const renderAI = () => (
        <div className="py-2 space-y-3">
            <div className="bg-white/5 border border-white/5 p-4 backdrop-blur-md">
            ...
            <div className="grid grid-cols-2 gap-3">
                ...
                <div className="glass p-4">
                    ...
                    <div className="text-sm font-black text-[var(--color-accent-cyan)] neon-glow-cyan px-2 py-0.5 bg-[var(--color-accent-cyan)]/10 w-fit">{aiThinking?.tags?.regime || 'Trending'}</div>
                </div>
            </div>
            <div className="text-[10px] text-[var(--color-text-muted)] font-medium italic flex items-center gap-2">
                <span className="w-1.5 h-1.5 bg-[var(--color-accent-cyan)] animate-pulse shadow-[0_0_5px_currentColor]" />
    ...
    return (
        <div className="glass flex flex-col h-full overflow-hidden animate-in">
